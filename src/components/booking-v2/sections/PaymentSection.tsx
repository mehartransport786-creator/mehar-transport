...
export function PaymentSection() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 sm:p-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#1B1E4F] to-[#2A2D5F] rounded-2xl p-6 border border-[#1B1E4F]/10">
        {/* Decorative background element */}
        <div className="absolute -right-6 -top-6 w-32 h-32 bg-white/5 rounded-full blur-2xl" />
        
        <div className="relative z-10 flex items-start gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#D9A63A]/20 flex items-center justify-center shrink-0">
            <Banknote className="w-6 h-6 text-[#D9A63A]" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-white mb-1">
              {isAr ? "الدفع نقداً بعد الرحلة" : "Cash / Card after the ride"}
            </h3>
            <p className="text-sm text-white/60 mb-4 leading-relaxed max-w-md">
              {isAr 
            ...
            <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg text-xs font-medium text-emerald-400">
