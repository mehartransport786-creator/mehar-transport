...
export function BookingConfirmation() {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-[0_8px_40px_rgb(0,0,0,0.06)] border border-gray-100 overflow-hidden w-full max-w-4xl mx-auto my-8">
      {/* Header Area */}
      <div className="bg-[#1B1E4F] p-8 md:p-12 text-center relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#D9A63A]/10 rounded-full blur-3xl" />
      ...
      <div className="p-6 md:p-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between bg-[#F8F9FC] rounded-2xl p-6 border border-gray-200 mb-10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-2xl md:text-3xl font-black text-[#1B1E4F] tracking-tight">
                {bookingId}
              </div>
            </div>
          </div>
          <button 
            onClick={handleCopy}
            className="flex items-center gap-2 bg-white border border-gray-200 px-5 py-2.5 rounded-xl text-sm font-semibold text-[#1B1E4F] hover:bg-gray-50 transition-colors shrink-0 w-full md:w-auto justify-center"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
          ...
          <div className="space-y-8">
            {/* Journey Details */}
            <section>
              <h3 className="text-lg font-bold text-[#1B1E4F] mb-4 border-b border-gray-100 pb-2">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs text-gray-500 mb-0.5">{isAr ? "المسار" : "Route"}</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs text-gray-500 mb-0.5">{isAr ? "تاريخ ووقت الرحلة" : "Date & Time"}</div>
                    <div className="font-semibold text-sm text-gray-900">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3 w-full">
                    <div>
                      <div className="text-xs text-gray-500 mb-0.5">{isAr ? "الاسم" : "Name"}</div>
                      <div className="font-semibold text-sm text-gray-900">{state.passengerInfo.name}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500 mb-0.5">{isAr ? "رقم الجوال" : "Mobile"}</div>
                      <div className="font-semibold text-sm text-gray-900" dir="ltr">{state.passengerInfo.phone}</div>
                    </div>
                    <div className="col-span-2">
                      <div className="text-xs text-gray-500 mb-0.5">{isAr ? "البريد الإلكتروني" : "Email Address"}</div>
                      <div className="font-semibold text-sm text-gray-900">{state.passengerInfo.email}</div>
                    </div>
          ...
          <div className="space-y-8">
            ...
            <section>
              ...
              <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="w-16 h-12 relative bg-white rounded-lg border border-gray-200 overflow-hidden shrink-0 flex items-center justify-center p-1">
                ...
                <div>
                  <div className="font-bold text-sm text-[#1B1E4F]">
            ...
            <section>
              ...
              <div className="space-y-3 bg-gray-50 p-5 rounded-xl border border-gray-100">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">{isAr ? "السعر الأساسي" : "Base Fare"}</span>
                ...
                {state.pricing.adjustments.map((adj, i) => (
                  <div key={i} className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-[#D9A63A]">+{adj.amount.toFixed(0)} SAR</span>
                  </div>
                ))}

                {state.pricing.taxAmount > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">{isAr ? "ضريبة القيمة المضافة" : "VAT (15%)"}</span>
                    <span className="font-medium">{state.pricing.taxAmount.toFixed(0)} SAR</span>
                  </div>
                )}
                
                <div className="pt-3 mt-3 border-t border-gray-200 flex justify-between items-center">
                  <span className="font-bold text-[#1B1E4F]">{isAr ? "الإجمالي" : "Total Amount"}</span>
                  <span className="text-xl font-black text-[#1B1E4F]">{state.pricing.totalIncludingTax.toFixed(0)} SAR</span>
                </div>
        ...
        {/* What Happens Next? */}
        <div className="bg-[#1B1E4F]/5 rounded-2xl p-6 md:p-8 mb-10 border border-[#1B1E4F]/10">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-[#D9A63A] mt-1.5 shrink-0" />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-6 border-t border-gray-100">
          <button 
            onClick={() => router.push('/')}
            className="w-full sm:w-auto px-8 py-3.5 bg-[#1B1E4F] text-white rounded-xl font-bold text-sm hover:bg-[#2A2D5F] transition-all flex items-center justify-center gap-2"
          >
          ...
          <button 
            ...
            className="w-full sm:w-auto px-8 py-3.5 bg-white text-[#1B1E4F] border-2 border-gray-200 rounded-xl font-bold text-sm hover:bg-gray-50 hover:border-gray-300 transition-all flex items-center justify-center gap-2"
          >
