...
export function PremiumVehicleCollection({ vehicles }: { vehicles: any[] }) {
  ...
  return (
    <section id="premium-collection" className="bg-background">
      {/* Header */}
      <div className="container-fluid pt-32 pb-16">
        ...
        {vehicles.map((vehicle, index) => {
          ...
          return (
            <div key={vehicle._id || vehicle.slug} className={`${bgColor} relative overflow-hidden`}>
              <div className="container-fluid py-24 lg:py-32">
                <div className={`flex flex-col gap-12 lg:gap-24 items-center ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                  
                  {/* Image Side */}
                  <div className="w-full lg:w-[55%] xl:w-[60%] animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both">
                    <div className="relative aspect-[4/3] rounded-[var(--radius-card)] overflow-hidden group shadow-[var(--shadow-luxury)] bg-muted border border-border">
                      ...
                      <Image 
                        ...
                        className="object-contain p-8 md:p-12 group-hover:scale-105 transition-transform duration-700 ease-out drop-shadow-[0_20px_30px_rgba(0,0,0,0.1)]"
                      />
                      {/* Interactive Gallery Hint */}
                      <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity duration-[var(--duration-instant)] ease-[var(--ease-out)] flex items-center justify-center backdrop-blur-[2px]">
                        <Link 
                          href={`/fleet/${vehicle.slug}`}
                          className="bg-background/95 text-primary px-8 py-3 rounded-[var(--radius-btn)] font-bold shadow-[var(--shadow-luxury)] hover:bg-background transition-colors transform translate-y-4 group-hover:translate-y-0 duration-[var(--duration-instant)] ease-[var(--ease-out)]"
                        >
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150 fill-mode-both">
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
                        <Link 
                          ...
                          className="w-full sm:w-auto text-center px-8 py-4 rounded-[var(--radius-btn)] font-bold transition-all duration-[var(--duration-instant)] ease-[var(--ease-out)] bg-primary text-primary-foreground hover:bg-secondary hover:text-secondary-foreground shadow-[var(--shadow-luxury)] hover:-translate-y-1"
                        >
                          {isAr ? "احجز الآن" : "Book Now"}
                        </Link>
                        <Link 
                          href={`/fleet/${vehicle.slug}`}
                          className="w-full sm:w-auto flex items-center justify-center gap-3 px-8 py-4 rounded-[var(--radius-btn)] font-bold transition-all duration-[var(--duration-instant)] ease-[var(--ease-out)] group text-primary hover:bg-primary/5 border border-border hover:-translate-y-1"
                        >
