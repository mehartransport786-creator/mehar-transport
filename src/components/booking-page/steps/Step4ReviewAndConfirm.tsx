  ...
  return (
    <div className="p-6 sm:p-8 md:p-10 lg:p-12">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-lg text-primary/40 font-medium" dir="rtl">المراجعة والتأكيد</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 bg-muted border-b border-gray-100 flex items-center gap-3">
            ...
            <h3 className="font-bold text-primary">Journey Summary</h3>
          </div>
              ...
              <div key={i} className="flex items-center gap-4">
                ...
                <div className="flex-1 space-y-2">
                  <div className="text-sm">
                    ...
                    <span className="font-semibold text-primary">{route.origin}</span>
                  </div>
                  <div className="text-sm">
                    <span className="text-xs text-gray-400 uppercase tracking-wider block">To</span>
                    <span className="font-semibold text-primary">{route.destination}</span>
                  </div>
                </div>
              </div>
            ))}
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-2.5 text-sm">
                ...
                <div>
                  ...
                  <div className="font-semibold text-primary">{state.dates.pickupDate} at {state.dates.pickupTime}</div>
                </div>
              </div>
              <div className="flex items-center gap-2.5 text-sm">
                ...
                <div>
                  ...
                  <div className="font-semibold text-primary">{state.passengerCount}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 bg-muted border-b border-gray-100 flex items-center gap-3">
            ...
            <h3 className="font-bold text-primary">Vehicle Summary</h3>
          </div>
            ...
            {state.vehicles.map((v, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-sm text-primary">{v.vehicleName}</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 bg-muted border-b border-gray-100 flex items-center gap-3">
            ...
            <h3 className="font-bold text-primary">Pricing Breakdown</h3>
          </div>
          <div className="p-6 space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Base Fare</span>
              <span className="font-semibold text-primary tabular-nums">{pricing.basePrice.toFixed(0)} SAR</span>
            </div>
            ...
            <div className="flex justify-between text-sm pt-2 border-t border-gray-100">
              ...
              <span className="font-semibold text-primary tabular-nums">{pricing.taxAmount.toFixed(0)} SAR</span>
            </div>
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    isSelected ? 'bg-secondary text-white' : 'bg-gray-50 text-gray-400'
                  }`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <div className="font-bold text-sm text-primary">{method.label}</div>
                    <div className="text-xs text-gray-400 truncate">{method.desc}</div>
                  </div>
                  {isSelected && <CheckCircle2 className="w-5 h-5 text-secondary ml-auto shrink-0" />}
                </button>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="flex flex-col items-center gap-2 p-4 bg-muted rounded-xl text-center">
      ...
      <div className="mt-10 flex justify-between items-center">
        ...
        <motion.button
          ...
          className="bg-secondary text-white px-8 sm:px-12 py-4 rounded-xl text-base font-bold hover:bg-secondary/80 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-3 shadow-lg shadow-secondary/30"
        >
          {isSubmitting ? (
            <>
