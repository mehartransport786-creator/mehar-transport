...
export function TrustBar() {
  ...
  return (
    <div className="relative z-20 -mt-16 lg:-mt-24 px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto">
      <div className="bg-card shadow-luxury rounded-3xl py-6 md:py-8 lg:py-10 px-4 sm:px-6 border border-border/50 backdrop-blur-xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 md:gap-12 sm:divide-x sm:divide-border sm:rtl:divide-x-reverse">
          {trustItems.map((item, index) => (
            <div key={index} className="flex flex-col items-center justify-center text-center space-y-3 sm:space-y-4 px-2 sm:px-4 group">
              <PremiumIcon 
                icon={item.icon} 
                size="md" 
