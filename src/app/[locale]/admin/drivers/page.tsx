...
import { mockDrivers } from "@/lib/admin-data";
import { Star, Phone, MapPin, TrendingUp, Eye, Edit3, MoreHorizontal } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm font-medium">
            {isAr ? `${mockDrivers.length} سائقين مسجلين` : `${mockDrivers.length} registered drivers`}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl border border-gray-100 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-[#1B1E4F]">{mockDrivers.filter(d => d.availability === "available").length}</div>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-[#1B1E4F]">{mockDrivers.filter(d => d.availability === "on_trip").length}</div>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-[#1B1E4F]">{(mockDrivers.reduce((a, d) => a + d.rating, 0) / mockDrivers.length).toFixed(1)}</div>
          </div>
        </div>
      </div>
