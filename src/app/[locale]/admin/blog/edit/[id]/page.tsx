...
import { getCategories, getTags, getAuthors, getPostById, updatePost } from '@/lib/actions/blog';
...
export default function EditBlogPost({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [cats, tgs, auths, post] = await Promise.all([
          getCategories(),
          getTags(),
          getAuthors(),
  ...
  const handleSubmit = async (status: string) => {
    try {
      ...
      await updatePost(params.id, dataToSubmit);
  ...
  if (loading) return <div className="p-12 text-center text-gray-500">Loading article editor...</div>;
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Editing "{formData.title}"</p>
          </div>
        </div>
