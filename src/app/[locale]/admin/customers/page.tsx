    ...
    <div className="space-y-8">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24 text-[#D9A63A]">
          ...
          <p className="mt-4 text-[#1B1E4F] font-semibold">{isAr ? "جاري التحميل..." : "Loading customers..."}</p>
        </div>
      ) : (
        <>
          {/* Quick Stats */}
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="text-3xl font-black text-[#1B1E4F]">{customers.length}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{isAr ? "عملاء VIP" : "VIP Customers"}</div>
              <div className="text-3xl font-black text-[#D9A63A]">{customers.filter(c => c.tags?.includes("VIP")).length}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{isAr ? "إجمالي الحجوزات" : "Total Bookings"}</div>
              <div className="text-3xl font-black text-[#1B1E4F]">{customers.reduce((a, c) => a + (c.totalBookings || 0), 0)}</div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{isAr ? "إجمالي القيمة" : "Lifetime Value"}</div>
              <div className="text-3xl font-black text-[#1B1E4F]">{(customers.reduce((a, c) => a + (c.lifetimeValue || 0), 0) / 1000).toFixed(0)}K</div>
            </div>
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {customers.map((customer) => (
                    <tr key={customer._id || customer.id} className="hover:bg-[#F8F9FC] transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center font-bold text-[#1B1E4F] hidden lg:table-cell">{customer.totalBookings || 0}</td>
                      <td className="px-6 py-4 font-bold text-[#1B1E4F]">{(customer.lifetimeValue || 0).toLocaleString()} SAR</td>
                      <td className="px-6 py-4 hidden lg:table-cell">
                        <div className="text-xs text-gray-500 space-y-1">
                          {customer.favoriteRoute && <div className="flex items-center gap-1.5"><MapPin className="w-3 h-3 text-[#D9A63A]" /> {customer.favoriteRoute}</div>}
                          {customer.preferredVehicle && <div className="flex items-center gap-1.5"><Car className="w-3 h-3 text-[#D9A63A]" /> {customer.preferredVehicle}</div>}
                        </div>
                      </td>
