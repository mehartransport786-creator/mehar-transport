...
export async function generateMetadata({
  ...
  // getAuthors() is cache()-deduplicated
  const authors = await getAuthors();
  const author = authors.find((a: Record<string, string>) => a.slug === params.slug);
  if (!author) return { title: 'Not Found' };

  return {
    title: `${params.locale === 'ar' ? (author as Record<string, string>).nameAr : (author as Record<string, string>).name} | Mehar Transport Blog`,
    description:
      params.locale === 'ar'
        ? (author as Record<string, string>).bioAr
        : (author as Record<string, string>).bio,
  };
}
...
export default async function AuthorPage({
  ...
  // getAuthors() is cache()-deduplicated — same call as generateMetadata
  const authors = await getAuthors();
  const author = authors.find((a: Record<string, string>) => a.slug === params.slug);
  if (!author) notFound();

  // Uses authorId compound index (authorId, status, createdAt) from Step 3
  const posts = await getPosts(
    {
      authorId: (author as Record<string, string>)._id,
      status: 'Published',
      language: params.locale,
    },
    { select: 'title slug excerpt featuredImage createdAt categoryId', limit: 24 }
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0F172A] pb-24">
      <div className="bg-primary py-20 lg:py-24">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col items-center">
            {(author as Record<string, string>).avatar ? (
              <img
                src={(author as Record<string, string>).avatar}
                alt={(author as Record<string, string>).name}
              ...
              <div className="w-32 h-32 rounded-full bg-secondary text-primary flex items-center justify-center font-bold text-4xl mb-6 border-4 border-white/20">
                {(author as Record<string, string>).name?.charAt(0) || 'M'}
              </div>
            )}
            <h1 className="text-4xl font-bold text-white mb-4">
              {isAr
                ? (author as Record<string, string>).nameAr
            ...
            <div className="flex items-center justify-center gap-4">
              {(author as Record<string, Record<string, string>>).socialLinks?.twitter && (
                <a
                  href={(author as Record<string, Record<string, string>>).socialLinks.twitter}
                  target="_blank"
              ...
              {(author as Record<string, Record<string, string>>).socialLinks?.linkedin && (
                <a
                  href={(author as Record<string, Record<string, string>>).socialLinks.linkedin}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/60 hover:text-white transition-colors"
                >
              ...
              {(author as Record<string, Record<string, string>>).socialLinks?.website && (
                <a
                  href={(author as Record<string, Record<string, string>>).socialLinks.website}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/60 hover:text-white transition-colors"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <h2 className="text-2xl font-bold mb-8 text-gray-900 dark:text-white">
          {isAr
            ? `مقالات بواسطة ${(author as Record<string, string>).nameAr}`
            : `Articles by ${(author as Record<string, string>).name}`}
        </h2>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-24 bg-white dark:bg-primary rounded-2xl border border-gray-100 dark:border-white/10">
            ...
            <p className="text-gray-500">
              {isAr
                ? 'لم ينشر هذا المؤلف أي مقالات بعد.'
